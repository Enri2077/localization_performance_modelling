...
from performance_modelling_py.metrics.localization_metrics import trajectory_length_metric, relative_localization_error_metrics, absolute_localization_error_metrics, env_absolute_localization_error_metrics
...
def compute_metrics(run_output_folder):
    ...
    # metrics_result_dict['absolute_localization_correction_error'] = env_absolute_localization_error_metrics(estimated_correction_poses_path, ground_truth_poses_path, ground_truth_map)
...
if __name__ == '__main__':
    run_folders = list(filter(path.isdir, glob.glob(path.expanduser("~/ds/performance_modelling/output/test_localization_2020-07-03/*"))))
